...
exports.createSampleEventJsonFile = async function (platform_event_name, num) {
    ...
    try {
        ...
        var meta = await
            sfdc.read_custom_objects(conn, events);
        winston.info('download platform event meta data');

        var fname = meta['fullName'];
        ...
        await
            files.create_file(file_name, JSON.stringify(root, null, 4));
